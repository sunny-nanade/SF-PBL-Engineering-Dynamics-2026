...
integration. The primary objective of this study was to design, implement,
and empirically evaluate a Simulation-Focused Problem-Based Learning
(SF-PBL) framework that integrates first-principles Newtonian and
Lagrangian mechanics with Python-based numerical simulation within an
Outcome-Based Education (OBE)-aligned curriculum. The framework was
embedded within a 15-week undergraduate Dynamic System
Modeling (DSM) course ($N$\,=\,53, B.Tech Mechatronics Engineering, Semester
IV, January--April 2026) at a single NBA-accredited institution in India. It
...
to computation as commonly reported breakthrough moments. Exhibition rubric scores
...
offers a replicable, empirically-evaluated model for computationally-integrated
...
These gaps distinguish the present work from three established pedagogical
models. Classical PBL, as originally operationalised in medical education
\citep{hmelo2004problem}, centres on open-ended clinical reasoning with no
structured computational pipeline; the primary learning artefact is a
diagnostic report, not a first-principles mathematical derivation verified
through simulation. Project-based and capstone-design approaches
\citep{dym2005engineering} typically foreground physical prototyping, treating
mathematical modelling as instrumental rather than as the core intellectual
product. Standalone computational modelling courses
\citep{justo2021enhancing} typically pre-specify the governing equations,
asking students to implement rather than derive them. SF-PBL integrates
these elements into a single cohesive framework where student-led
derivation, computational simulation, and authentic public defence form an
interdependent sequence. Recent work on cross-disciplinary team formation in
computationally-intensive XR engineering courses at the same institution
\citep{nanade2026cross} further underscores the pedagogical value of
structured active-learning interventions in engineering curricula.
...
pre-to-post learning gains ($d$\,$>$\,1.0) from a mixed-methods study
within a single cohort ($N$\,=\,53); and (4)~a characterisation of emergent human--AI
...
Table~\ref{tab:synthesis} summarises the alignment between these theoretical
foundations, the SF-PBL phases, and the assessment instruments employed.

\begin{table}[h]
\caption{\textbf{Theoretical--Pedagogical Synthesis:} Mapping of theoretical
foundations to SF-PBL phases, E-T-C stages, and assessment instruments.}
\label{tab:synthesis}
\begin{tabular}{lllll}
\toprule
\textbf{Theoretical Pillar} & \textbf{SF-PBL Phase} & \textbf{E-T-C} & \textbf{Assessment} \\
\midrule
Hmelo-Silver PBL Cycle         & Phase 1: System ID       & Explain   & Project proposal  \\
Bandura Self-Efficacy (Mastery) & Phases 2--3: Derivation + Sim. & Try & SE scale (pre--post) \\
Hake Interactive Engagement    & Phases 3--4: Sim. + Validation & Challenge & MCQ + Exhibition  \\
Vygotsky ZPD / Scaffolding     & All phases (fading support)    & E-T-C     & Reflection journals \\
Kolb Experiential Learning     & Phase 4: Exhibition            & Challenge & 6-judge panel      \\
\bottomrule
\end{tabular}
\end{table}
...
In the NBA/OBE framework, Programme Outcome (PO) attainment is evidenced
through Course Outcome performance. COs 1--4 map jointly to PO1 (Engineering
Knowledge) and PO2 (Problem Analysis), assessed through the mathematical
derivation and simulation phases of SF-PBL. CO5 additionally maps to PO5
(Modern Tool Usage), assessed through the Python implementation requirement.
The six-judge exhibition rubric spans all five COs, with each judge
dimension providing evidence for one or more POs: the Mathematics judge
(NG) assesses CO1--CO4 directly, while the Industry~4.0 judge (MK) assesses
CO5 through evaluation of computational implementation quality. Course
Outcome attainment is analysed in Section~4.6 using both direct (MCQ) and
indirect (self-efficacy) assessment evidence.
...
Institutional Research and Ethics Review Committee at Mukesh Patel School of
Technology Management and Engineering, SVKM's NMIMS, Mumbai, India. All 53
student participants were adult second-year undergraduate engineering students
aged 18 years or older (age range: 19--21 years); no minors (under 18 years of
age) participated in this study. Participation in the research survey was
voluntary and anonymised, and course grades were not affected by survey
participation. Written informed consent to participate in this study was
provided by all 53 student participants prior to pre-survey administration,
and written informed consent was obtained from all six external expert
evaluators for the publication of their domain evaluations and professional
attributions. The two registered students who declined consent are excluded
from all analyses; $N$\,=\,53 applies throughout this paper. As this is a
single-cohort, single-institution study, findings should be interpreted within
this specific institutional and cultural context.
...
Because each of the six judges evaluated a distinct professional dimension
(e.g., manufacturing, patent, mathematics, communication), classical pairwise
agreement metrics such as Cohen's $\kappa$ are not directly applicable---the
judges were not rating the same construct. Instead, inter-rater consistency
was assessed using two complementary metrics: (a)~Cronbach's $\alpha$
computed over the six judges' standardised percentage scores, measuring
composite reliability of the panel as a whole; and (b)~Kendall's coefficient
of concordance ($W$), which tests whether the six judges' rank-orderings of
student performance are statistically concordant. These were computed using
SciPy~1.11 and are reported in Section~4.5.
...
Inter-rater consistency across the six expert domains was assessed using
two complementary metrics. Cronbach's $\alpha$, computed over the
standardised percentage scores of the six judges, yielded
$\alpha$\,=\,0.829, indicating high composite reliability. Kendall's
coefficient of concordance was $W$\,=\,0.448,
$\chi^2$(52)\,=\,139.63, $p$\,$<$\,.0001, confirming statistically
significant agreement in the judges' rank-ordering of student performance
across all six professional domains.

\subsection{Course Outcome Attainment (OBE)}
\label{sec:coattainment}

Table~\ref{tab:coattain} presents Course Outcome attainment evidence from
two independent instruments: direct assessment (MCQ items PK1--PK5,
percentage of students answering correctly) and indirect assessment
(self-efficacy items, percentage of students rating $\geq$\,4 on the
5-point scale, i.e., ``Agree'' or ``Strongly Agree'').

\begin{table}[h]
\caption{\textbf{Course Outcome Attainment:} Direct (MCQ) and Indirect
(Self-Efficacy) evidence, pre- and post-intervention ($N$\,=\,53).}
\label{tab:coattain}
\begin{tabular}{llrrrr}
\toprule
\textbf{CO} & \textbf{Bloom's} &
\multicolumn{2}{c}{\textbf{Direct (MCQ \% correct)}} &
\multicolumn{2}{c}{\textbf{Indirect (SE $\geq$ 4, \%)}} \\
\cmidrule(lr){3-4} \cmidrule(lr){5-6}
 & & Pre & Post & Pre & Post \\
\midrule
CO1: Kinematics      & Apply (L3)    & 28.3 & 39.6  &  3.8 & 67.9 \\
CO2: FBDs            & Analyse (L4)  &  9.4 & 30.2  &  3.8 & 69.8 \\
CO3: Newton's EOMs   & Analyse (L4)  & 81.1 & 96.2  &  0.0 & 75.5 \\
CO4: Lagrangian      & Evaluate (L5) &  9.4 & 71.7  &  3.8 & 73.6 \\
CO5: Python/ODE      & Create (L6)   & 67.9 & 90.6  &  4.9 & 71.3 \\
\bottomrule
\multicolumn{6}{l}{\small Direct: PK1--PK5 mapped to CO1--CO5.
Indirect: SE1--SE9 mapped to CO1--CO5 (SE5--SE9 averaged for CO5).}
\end{tabular}
\end{table}

CO4 (Lagrangian mechanics, Bloom's L5) showed the largest direct gain
(+62.3 percentage points), consistent with its position as the highest
Bloom's level among CO1--CO4 and the competency most directly activated by
SF-PBL Phase~2. CO3 (Newton's EOMs) achieved near-ceiling post-intervention
attainment (96.2\%), reflecting the strong scaffolding provided by the E-T-C
model for this foundational competency.

A noteworthy pattern emerges for CO1 and CO2: direct MCQ attainment
remains modest (39.6\% and 30.2\% respectively), while indirect
self-efficacy attainment is strong (67.9\% and 69.8\%). This discrepancy
suggests that students developed procedural confidence through the SF-PBL
hands-on experience, while the MCQ items for kinematics (PK1) and FBD
construction (PK2) assessed deeper conceptual discrimination that the
intervention did not fully address within the 15-week timeframe. This
finding identifies CO1 and CO2 as targets for additional scaffolding in
...
All findings reported below are interpreted within the constraints of a
single-group pretest-posttest design
\citep{campbell1963experimental}. In the absence of a concurrent control
group, observed pre-to-post gains cannot be exclusively attributed to the
SF-PBL intervention; maturation effects, assessment familiarity, and
general course exposure represent plausible alternative explanations.
The following discussion therefore interprets the data as evidence
consistent with, though not proof of, the framework's pedagogical
effectiveness.
...
meta-analyses. Three factors contextualise the magnitude of the present
finding and warrant careful interpretation. First, the narrow pre-survey
SD ($SD$\,=\,0.21 on a 5-point scale) reflects baseline range
restriction where students uniformly reported low-to-moderate confidence
prior to any dynamics instruction---a characteristic often observed in
engineering programmes in India where intake pathways are
standardised \citep{streveler2008learning}---which mathematically amplifies
the standardised effect size. Second, alternative effect size formulations
yield varying but consistently large estimates: the within-subjects
$d_z$ (using the SD of paired differences) is 1.91, and Glass's
$\Delta$ (using only $SD_{\text{pre}}$) is 4.16. All three formulations
confirm a genuine, substantial shift, but the variation across metrics
underscores the sensitivity of standardised effect sizes to baseline
variance characteristics. Third, the absolute mean gain ($+$0.88 points,
95\%~CI [0.75, 1.00]) is the most practically informative metric:
it represents a reliable, cohort-wide shift from approximately ``Neutral''
to ``Agree'' on domain-specific competency statements, a meaningful
pedagogical outcome regardless of the standardised metric employed.
Readers should therefore interpret the effect size magnitude with awareness
of the baseline variance characteristics of this specific cohort.

Second, the differential gain pattern across items is theoretically
noteworthy. The two largest-gain items---SE8 (PID Tuning, $d$\,=\,2.22)
...
This pattern is consistent with the interpretation that the computational
integration mechanism activated the intended outcomes through Bandura's
\citeyearpar{bandura1997self} mastery experience pathway, rather than
producing undifferentiated, uniform gains.
...
farrokhnia2024swot, nanade2026aiethics} has emphasised the need for
curriculum-embedded pedagogical structures that contextualise AI use within
disciplinary reasoning and responsible engineering practice. The SF-PBL
framework provides one such structure: by requiring that simulation emerge from
...
``AI use reflection'' component to OE3 \citep{nanade2026aiethics}, asking
students to articulate which aspects of their project they chose to complete
without AI and why.
...
indicates meaningful score discrimination: the 43-point range separates
...
\subsection{The Breakthrough Theme: A Widely Observed Learning Landmark}
...
size, suggesting it may represent a \textbf{common learning landmark} in
...
experience and parallels findings on real-time adaptive feedback in
mechatronics and robotics programming courses \citep{nanade2026adaptive}:
the simulation serves as visceral confirmation of mathematical
...
designed to be domain-adaptable; its transferability to related subjects such
as thermodynamics, fluid mechanics, structural analysis, and control systems
warrants empirical investigation, as the core pedagogical logic of ``derive
first, simulate second, validate third'' is broadly applicable in computational
engineering education. Second, the \textbf{E-T-C lecture model} provides
active-learning scaffolding within existing contact hours without requiring
timetable restructuring, making adoption feasible for institutions with rigid
curricula. Third, the \textbf{six-judge external rubric with CO$\rightarrow$PO
traceability} provides an accreditation-compliant (NBA/ABET) authentic
assessment instrument with inter-professional credibility that faculty-only
...
desirability bias; while inter-rater composite reliability and rank
concordance were high (Cronbach's $\alpha$\,=\,0.829, Kendall's $W$\,=\,0.448,
$p$\,$<$\,.0001), the multi-domain architecture means judges evaluated distinct
professional constructs rather than identical criteria, precluding classical
pairwise consensus metrics ($\kappa$); and the narrow pre-survey SE standard
deviation reflects baseline range restriction. \textit{Timeframe}: the study
...
institutions and engineering disciplines using a quasi-experimental or
controlled comparison design---a companion study at the same institution
\citep{nanade2026cross} demonstrates the methodological feasibility of such
comparative evaluations in computationally-intensive engineering education;
(2)~extend the inter-rater reliability assessment by evaluating intraclass
correlation coefficients (ICC) across fine-grained sub-rubric criteria;
(3)~conduct longitudinal follow-up at six and twelve months to assess whether
self-efficacy and conceptual gains persist into subsequent core courses and
capstone projects; (4)~investigate the ``derivation-to-simulation
breakthrough'' as a targetable threshold concept through think-aloud
protocols and process tracing; and (5)~develop and validate an AI use
reflection instrument that characterises productive human--AI
complementarity in PBL contexts.

%%% ═══════════════════════════════════════════════════════════════════════
\section{Conclusion}
%%% ═══════════════════════════════════════════════════════════════════════

This study fulfilled its primary objective---to design, implement, and
empirically evaluate the Simulation-Focused Problem-Based Learning (SF-PBL)
framework---through a mixed-methods evaluation in an undergraduate
engineering dynamics course ($N$\,=\,53) at a single NBA-accredited
institution within a single-cohort context. The framework operationalised
a four-phase computational pipeline---System Identification, Mathematical
Modelling, Python-based ODE Simulation, and Validation \&
Communication---within a 15-week OBE-aligned course, assessed by a six-judge
external expert panel spanning five professional domains, and underpinned
throughout by the Explain-Try-Challenge instructional model.
...
The SF-PBL framework offers a replicable, empirically-evaluated, and
OBE-compliant pedagogical model whose broader applicability warrants
controlled replication across diverse institutional contexts.

%%% ─── BACK MATTER ───────────────────────────────────────────────────────

\section*{Ethics Statement}
The studies involving human participants were reviewed and approved by the
Institutional Research and Ethics Review Committee, Mukesh Patel School of
Technology Management and Engineering, SVKM's NMIMS, Mumbai, India. All
student participants were adult undergraduate engineering students aged 18
years or older (age range: 19--21 years); no minors (under 18 years of age)
participated in this study. Written informed consent to participate in this
study was provided by the participants prior to pre-survey administration.
Written informed consent was obtained from the individual(s) for the
publication of any potentially identifiable images or data included in this
article.
